...
void eval(char *cmdline) 
{
    ...
    if(pid == 0){
        ...
        if(setpgid(0,0)<0){;//put child process to background
            perror("SETPGID ERROR");
            exit(0);
        }
        int res = execve(argv[0],argv,environ);
        if(res == -1){
    ...
    if(!bg){
        ...
        sigprocmask(SIG_UNBLOCK,&mask_full,NULL);
        waitfg(pid);
    }
    else{
...
void do_bgfg(char **argv) 
{
    ...
    if(sscanf(argv[1],"%%%d",&id) > 0){
        ...
        printf("%s:argument mush be a PID or jobid\n",argv[0]);
        return;
    }
...
void waitfg(pid_t pid)
{
    sigset_t mask_SIGCHLD,mask_pre;
    sigemptyset(&mask_SIGCHLD);
    sigaddset(&mask_SIGCHLD,SIGCHLD);
    while(1){
        sigprocmask(SIG_BLOCK,&mask_SIGCHLD,&mask_pre);
        if(fgpid(jobs) == 0)
            break;
        sigsuspend(&mask_pre);
        sigprocmask(SIG_UNBLOCK,&mask_SIGCHLD,NULL);
...
void sigchld_handler(int sig) 
{
    ...
    int states = 0;
    while(1){
        pid = waitpid(-1,&states,WNOHANG|WUNTRACED);
        if(pid <= 0) break;
